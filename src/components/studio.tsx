...
export function Studio() {
  ...
  const packFitRef = useRef(packFit);
  packFitRef.current = packFit;
  ...
  function packMates(id: string, pack = false) {
    ...
    if (!hit) return [];
    if (isMark(id)) return skillPicksRef.current.filter((item) => item.id === id);
    const asPack = pack || packFitRef.current === "fit" || hit.group === "pack";
    if (!asPack) return skillPicksRef.current.filter((item) => item.id === id);
  ...
  function nudgeSelected(dx: number, dy: number) {
    ...
    if (pickedSkill && isMark(pickedSkill)) {
      setSkillPicks((cur) => {
        const next = cur.map((item) => {
    ...
    if (pickedSkill && packMates(pickedSkill).length <= 1) {
      setSkillPicks((cur) => {
        const next = cur.map((item) => {
          if (item.id !== pickedSkill || item.x == null || item.y == null) return item;
          const mark = Math.max(12, (item.size ?? skillSize) * (item.scale ?? 1));
  ...
  function fitPackToPlate(w: number, h: number, mode: "fit" | "room" | "desk" = packFit, fillAll = false) {
    ...
    setSkillSize(cell);
    const tagged = laid.map((item) => ({ ...item, group: mode === "fit" ? "pack" : undefined }));
    const next = [...tagged, ...marks];
    skillPicksRef.current = next;
    setSkillPicks(next);
  ...
  function applyPackFit(mode: "fit" | "room" | "desk") {
    ...
    const hasSkills = skillPicksRef.current.some((item) => !isMark(item.id));
    fitPackToPlate(box.width, box.height, mode, mode === "fit" ? !hasSkills : false);
  }
  ...
  function paintHud(ctx: CanvasRenderingContext2D, w: number, h: number, sx = 1, sy = 1) {
    ...
    for (const line of extras) {
      ...
      if (!placed) {
        if (line.id !== "streamer") {
          textPosRef.current[line.id] = { x: x / Math.max(0.001, sx), y: y / Math.max(0.001, sy) };
        }
        ny = y + size + 6;
  ...
  function applyStampScale(skillId: string, nextScale: number) {
    ...
    const mates = packMates(skillId);
    const item = mates.find((row) => row.id === skillId) ?? mates[0];
    ...
    const ratio = scale / Math.max(0.01, old);
    if (mates.length <= 1) {
      ...
      const b = packBounds(mates, skillSize);
      const cx = b ? (b.minX + b.maxX) / 2 : item.x;
      const cy = b ? (b.minY + b.maxY) / 2 : item.y;
      skillPicksRef.current = skillPicksRef.current.map((row) => {
        const mate = mates.find((m) => m.id === row.id);
        if (!mate || mate.x == null || mate.y == null) return row;
        const prev = mate.scale ?? 1;
        const sc = Math.min(3, Math.max(0.4, prev * ratio));
        const base = mate.size ?? skillSize;
        const ox = mate.x + (base * prev) / 2;
        const oy = mate.y + (base * prev) / 2;
        const nx = cx + (ox - cx) * ratio - (base * sc) / 2;
        const ny = cy + (oy - cy) * ratio - (base * sc) / 2;
        return {
          ...row,
          scale: sc,
          x: Math.max(0, Math.min(size.width - base * sc, nx)),
          y: Math.max(0, Math.min(size.height - base * sc, ny)),
        };
      });
    }
    requestPaint();
      ...
      <section id="desk" className="page-band scroll-mt-24 py-6">
        ...
        >
          ...
          {goldStar ? (
            <span
              aria-hidden="true"
              className="pointer-events-none absolute right-2 top-2 text-[10px] leading-none text-[#39ff6a]"
            >
              ★
            </span>
          ) : null}
          <canvas
            ...
            onPointerDown={(e) => {
              ...
              if (skillHit && skillHit.x != null && skillHit.y != null) {
                ...
                const pack =
                  !isMark(skillHit.id) &&
                  (e.shiftKey || packFitRef.current === "fit" || skillHit.group === "pack");
          ...
          {pickedSkill && SKILLS.some((skill) => skill.id === pickedSkill) ? (
            <label className="flex h-8 items-center gap-1 text-[10px] text-muted">
              ...
              <input
                ...
                inputMode="numeric"
                className="h-8 w-12 rounded-sm border border-[#39ff6a]/35 bg-[#0b0b0b] px-1 text-sm text-parchment outline-none ring-0 focus-visible:border-[#39ff6a]"
                onChange={(e) => {
        ...
        <div className="lg:border-r lg:border-[#39ff6a]/20">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              <button
                ...
                onClick={() => {
                  setPackFit("desk");
                  const box = BANNER_SIZES.find((s) => s.id === sizeIdRef.current) ?? size;
                  fitPackToPlate(box.width, box.height, "desk", true);
                }}
              >
                All skills
              </button>
                ...
                <>
                  {([0.75, 1, 1.35] as const).map((mult, i) => (
                    <button
                      ...
                      onClick={() => {
                        applyStampScale(pickedSkill, mult);
                      }}
                    >
                      {["S", "M", "L"][i]}
                    </button>
                ...
                return (
                  <div key={skill.id} className="flex min-w-0 flex-col items-center gap-0.5">
                    ...
                    <input
                      ...
                      placeholder="—"
                      maxLength={3}
                      className="h-6 w-8 rounded-sm border border-[#39ff6a]/35 bg-[#0b0b0b] px-0 text-center text-[10px] tabular-nums text-parchment outline-none ring-0 focus-visible:border-[#39ff6a]"
                      onChange={(e) => {
                        const next = sanitizeSkillLevel(e.target.value, cap);
                        setSkillPicks((cur) => {
            ...
            {MARKS.filter((mark) => mark.editions.includes(skillPack)).map((mark) => {
              ...
              return (
                <button
                  ...
                  className={`flex size-12 items-center justify-center rounded-none bg-transparent p-0 [touch-action:manipulation] ${on ? "outline outline-1 outline-[#39ff6a]" : ""}`}
                >
              ...
              .map((mark) => {
                ...
                return (
                  <button
                    ...
                    className={`flex size-12 items-center justify-center bg-transparent p-0 ${on ? "outline outline-1 outline-[#39ff6a]" : ""}`}
                  >
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5 p-1 md:grid-cols-4">
          <label className="text-[10px] text-muted">
            ...
            <input
              ...
              spellCheck={false}
              autoComplete="off"
              className="mt-0.5 min-h-11 w-full rounded-sm border border-[#39ff6a]/35 bg-[#0b0b0b] px-1 text-base text-parchment outline-none ring-0 focus-visible:border-[#39ff6a]"
            />
          </label>
          <label className="text-[10px] text-muted">
            Clan
          ...
          <button
            ...
            className={`box-border size-11 shrink-0 rounded-full border p-0 text-[10px] leading-none ${
              bannerCaps
                ...
                : "border-[#39ff6a]/55 bg-[#0b0b0b] text-muted"
            }`}
          >
            Caps
          </button>
