//#region node_modules/.nitro/vite/services/ssr/assets/locations-DGhympWJ.js
var GODS = [
	"Saradomin",
	"Zamorak",
	"Guthix",
...
function townPlateSrc(id) {
	return SHARP_TOWN[id] ? asset(SHARP_TOWN[id]) : void 0;
}
function townStillLine(id) {
	return TOWN_LINE[id] ?? "In-world still. Same file on the card and the plate.";
}
function townRegionHead(region) {
	return region.replace(/\s·\sOSRS$/, "").replace(/\s·\sRuneScape$/, "");
}
function townHasStill(loc) {
...
function loc(id, name, region, god, edition, kind, extra = {}) {
	const viewA = extra.viewA ?? (kind === "boss" ? asset(`/stills/${edition === "OSRS" ? "osrs" : "rs3"}/boss-${id}.jpg`) : townPlateSrc(id) ?? asset(`/locations/${id}.jpg`));
	const viewB = extra.viewB && stillAllowed(extra.viewB, edition) ? extra.viewB : kind === "town" ? asset(`/locations/${id}-nightstone.jpg`) : void 0;
...
//#endregion
export { migrateBannerSizeId as a, townPlateSrc as c, RULES as i, townRegionHead as l, GODS as n, stillAllowed as o, LOCATIONS as r, townHasStill as s, BANNER_SIZES as t, townStillLine as u };
