...
import { h as useVisibleNow, n as Route, s as BackLink } from "./router-DXmYNu76.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/towns._id-BL-lBjN8.js
...
var FALLBACK = [{ role: "Guard" }];
function citizenPool(id, game) {
	const pool = game === "osrs" ? OSRS$1[id] : RS3$1[id];
...
var OSRS = {
	...
	osrsprif: ["Elven door-warden: the city grew back from a seed. Song of the Elves is the constitution here.", "Crystal-stall: eight towers, one voice. No hour-clan on this card."],
	osrsedge: [
		"Guard: monastery west, ditch north. This is still a village that pretends it is a frontier.",
	...
	osrsdraynor: ["Market guard: willows, bank, and a village that remembers worse nights."]
};
var RS3 = {
	lumbridge: [
	...
	falador: ["White Knight: the square still performs order. The walls have taken hits this client recorded.", "Rising Sun keep: same postcode for piety and farce."],
	varrock: ["Palace clerk: the crown speaks. The Exchange answers in numbers.", "South-east stall: unrest is weather. I do not sell it."],
	prifddinas: ["Tower door: the hour will turn. This desk will not name the clan.", "Crystal-stall: Voice of Seren is a pulse. I sell thread, not a forecast."],
	edgeville: ["Guard: the ditch is still a dare in this client. The village keeps its voice down.", "Monk: the hill holds. I do not price the wilderness."]
};
var FALLBACK_OSRS = ["A guard: this street is the 2007-era plan. No later crater lives here.", "A stallholder: I sell what the client shows. The wiki keeps the hour."];
var FALLBACK_RS3 = ["A clerk: this street carries later-age weather only where the live page says so.", "A stallholder: I do not invent raids. Official news wins."];
function streetTalk(id, game) {
	const pool = game === "osrs" ? OSRS[id] ?? FALLBACK_OSRS : RS3[id] ?? FALLBACK_RS3;
	return pool.length ? pool : FALLBACK_OSRS;
}
