...
import { t as OfficialSites } from "./official-sites-CbG3wdq_.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/streamers-Dlozv3AO.js
...
function Row({ row, live, showGame, viewers }) {
	const badge = row.twitch ? live[row.twitch.toLowerCase()] ?? null : null;
	const count = row.twitch ? viewers?.[row.twitch.toLowerCase()] : void 0;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
			...
			children: [
				...
				badge === "live" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
					className: "ml-2 rounded-sm border border-parchment px-1.5 py-0.5 text-[10px] text-parchment",
					children: ["Live", typeof count === "number" ? ` · ${count}` : ""]
				}) : null,
...
function StreamersPage() {
	...
	const [viewers, setViewers] = (0, import_react.useState)({});
	const [probe, setProbe] = (0, import_react.useState)("down");
	const [q, setQ] = (0, import_react.useState)("");
	const [cut, setCut] = (0, import_react.useState)("all");
	(0, import_react.useEffect)(() => {
		...
		poll();
		const id = window.setInterval(poll, 6e4);
		const onVis = () => {
	...
	const gameOk = (row) => cut === "all" || cut === "live" || row.game === cut;
	const official = CHANNELS.filter((row) => row.official && hrefs(row).length && match(row) && gameOk(row));
	const foundation = CHANNELS.filter((row) => row.era === "foundation" && hrefs(row).length && match(row) && gameOk(row) && cut !== "live").sort((a, b) => a.name.localeCompare(b.name));
	...
	const liveNow = livePeople.filter((row) => match(row) && (cut === "all" || cut === "live" || row.game === cut)).sort((a, b) => a.name.localeCompare(b.name));
	const liveBadges = {};
	for (const row of liveNow) if (row.twitch) liveBadges[row.twitch.toLowerCase()] = "live";
	const osrsRest = CHANNELS.filter((row) => cut !== "live" && cut !== "rs3" && row.game === "osrs" && !row.official && row.era !== "foundation" && hrefs(row).length && match(row) && !(row.twitch && liveHandles.has(row.twitch.toLowerCase()))).sort((a, b) => a.name.localeCompare(b.name));
	const rsRest = CHANNELS.filter((row) => cut !== "live" && cut !== "osrs" && row.game === "rs3" && !row.official && row.era !== "foundation" && hrefs(row).length && match(row) && !(row.twitch && liveHandles.has(row.twitch.toLowerCase()))).sort((a, b) => a.name.localeCompare(b.name));
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", {
			...
			children: [
				...
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					...
					children: [
						...
						type: "button",
						"aria-pressed": cut === id,
						className: `min-h-11 rounded-md border px-3 text-xs ${cut === id ? "border-parchment bg-surface text-parchment" : "border-line text-muted"}`,
						onClick: () => setCut(id),
						children: id === "all" ? "All" : id === "osrs" ? "Old School" : id === "rs3" ? "RuneScape" : "Live only"
					}, id))
				})
			]
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(OfficialSites, {}),
