...
//#region node_modules/.nitro/vite/services/ssr/assets/hiscores-Bu69iAvQ.js
...
function HiscoresLookup({ name: boundName, onName, onLevels, bare = false }) {
	...
	async function lookup(nextEdition = editionRef.current) {
		...
		try {
			...
			setStatus(next.length ? "Found" : nextEdition === "OSRS" ? "Not on the Old School Hiscores. The name is still on the desk." : "Not on the RuneScape Hiscores. The name is still on the desk.");
		} catch {
			if (id !== seq.current) return;
			setStatus("Boards are busy.");
		} finally {
