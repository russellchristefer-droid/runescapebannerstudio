    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (pathOnly === "/api/twitch-live") {
            const mod = (await server.ssrLoadModule("/src/lib/live.server.ts")) as {
              fetchTwitchLiveBoard: (
                logins: string[],
              ) => Promise<{ off?: boolean; ok: boolean; rows: { handle: string; live: boolean | "unknown" }[] }>;
            };
          ...
          const mod = (await server.ssrLoadModule("/src/lib/live.server.ts")) as {
            fetchTwitchLive: (logins: string[]) => Promise<Record<string, string>>;
          };
          const live = await mod.fetchTwitchLive(logins);
          res.statusCode = 200;
          res.setHeader("content-type", "application/json; charset=utf-8");
...
function stillsCachePlugin(): Plugin {
  return {
    name: "app-builder:stills-cache",
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const path = (req.url ?? "").split("?")[0];
...
function hiscoresProxyPlugin(): Plugin {
  return {
    name: "app-builder:hiscores-proxy",
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        const rawUrl = req.url ?? "";
        const [pathOnly, query = ""] = rawUrl.split("?");
        if (pathOnly !== "/api/hiscores") {
          next();
          return;
        }
        try {
          const params = new URLSearchParams(query);
          const mod = (await server.ssrLoadModule("/src/lib/hiscores.server.ts")) as {
            fetchHiscoreLite: (
              edition: string,
              player: string,
            ) => Promise<{ text?: string; error?: string; status: number }>;
          };
          const result = await mod.fetchHiscoreLite(
            params.get("edition") ?? "OSRS",
            params.get("player") ?? "",
          );
